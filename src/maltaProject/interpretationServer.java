...
public class interpretationServer extends Thread {
    ...
    // ---Database Location---//
    String database = null; // Database address is passed to interpretationServer() from
                            // TelnetServer(). To modify this path, edit TelnetServer()
    ...
    String direction = "";
    String northRoom, eastRoom, southRoom, westRoom, upRoom, downRoom; // The program loads all
                                                                       // rooms adjacent to the
                                                                       // users current position.
    static int numofItems;
    boolean show = true; // decides whether to show the intro screen. For example if a password is
                         // incorrect, the intro screen will not be shown.
    ...
    public void ZaosClient(Socket s, String databaseLoc) throws IOException {
        ...
        // If any of the above calls throw an exception, the caller is responsible for closing the
        // socket. Otherwise the thread will close it.
        start(); // Calls run
    }
        ...
        while (login == false) {
            ...
            if (str.equals("q")) {
                ...
                if (failLogin == false) {
                    out.println("");
                    out.println("");
                    out.println("Password:" + (char) 27 + "[8m"); // This ansi code will make the
    ...
    public void characterLogin() {// Character Login Page
        ...
        if (str.equals("c")) {
            ...
            room();
            /**
             * out.println(""); out.println("Name of the Character that you would like to play:");
             * str = usrTxt.nextLine(); str = str.trim(); str = str.toLowerCase(); try{
             * FileInputStream fstream = new FileInputStream(database + "/characters/"+user);
             * DataInputStream in = new DataInputStream(fstream); BufferedReader br = new
             * BufferedReader(new InputStreamReader(in)); while(loadChar==false){ while
             * (name.equals("<none>")){ strLine = br.readLine(); if (strLine.equals(str)){ name =
             * strLine; } } } in.close(); } catch (Exception e){
    ...
    public void room() {// Prints Description and exits of a room - called by interpretUsr()
        ...
        out.println((char) 27 + "[1m" + "---" + Title + "---" + (char) 27 + "[39m"); // Prints the
                                                                                     // title of the
                                                                                     // room
        out.println("");
            ...
            if (str.equals("n") | str.equals("north")) {
                ...
                if (southRoom.equals("none")) {
                    ...
                    try {
                        ...
                        try {
                            FileInputStream fstream2 = new FileInputStream(database + "/rooms/" + location + "/" + location + ".s");
                            DataInputStream in = new DataInputStream(fstream2);
                            BufferedReader br = new BufferedReader(new InputStreamReader(in));
                            out.println();
                            out.println(br.readLine());
